...
int buscarPorLegajo(eAlumno listadoDeAlumnos[], int tam, int legajoIngresado){
    int i;
    int retorno = 0;
    for (i = 0; i <tam ; i ++ ){
    if (listadoDeAlumnos[i].legajo==legajoIngresado){
        retorno = 1;
    }
...
void modificarAlumno(eAlumno listadoDeAlumnos[], int cantidad, int tam){
    int legajoIngresado;
    int index;
    printf("Ingresar legajo a modificar");
    scanf("%d", &legajoIngresado);
    index = buscarPorLegajo(legajoIngresado);

    if (index!= -1){
        mostrarUnAlumno(listadoDeAlumnos[index])
        gets(nombre);
